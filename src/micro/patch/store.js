...
// 一个工厂函数，返回localStorage指定命名空间下对应的store对象。
function dbFactory (databaseName = 'database', router = {}) {
    /**
...
// 提供子应用动态注册store modules的方法。
function register (appName = '', options = {}) {
    mustBeRegisteredApp(appName, 'Store modules cannot be registered.')
    let { modules } = options
    store.registerModule(appName, modules)
}

// 提供子应用动态移除已注册的store modules的方法。
function unregister (appName = '', options = {}) {
    mustBeRegisteredApp(appName, 'Store modules cannot be unregistered.')
